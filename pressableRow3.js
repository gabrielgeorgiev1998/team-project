...
// pressableRow3 is used for the market space (MarketSpaceTableComponent.js). It works the same as
// pressableRow2: when a user presses the row they are given the option to book space. 
// However the query to do so is different.
...
export default class PressableRow extends Component {
    ...
    handleSelectSpace=async(rowData)=> {
       ...
       { var spaceArray = []
        ...
        // query 'marketSpaces' collection (collection of free market spaces) in database and delete selected space from that collection
        let getSpaceFreed =  await firebase.firestore().collection('marketSpaces').doc(rowData[1].toString()).get().then(doc=>{
            spaceArray = doc.data().Dates
        });
        var index = spaceArray.indexOf(rowData[1]);
        spaceArray.splice(index, 1);
        let updateDoc = await firebase.firestore().collection('marketSpaces').doc(rowData[1].toString()).update({
            Dates:spaceArray
        })
